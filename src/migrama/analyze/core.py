"""
Core analysis functionality for migrama analyze.
...
from ..core import CellposeCounter, Cropper, CropperParameters
from ..core.io.h5_io import (
    append_analysis_hdf5,
    get_available_fovs,
...
class Analyzer:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(f"Successfully initialized Analyzer with cells: {cells_path} and H5: {h5_path}")
    ...
    def _init_cropper(
        ...
        try:
            ...
            logger.debug(f"Initialized cropper with {self.cropper.n_fovs} views and {self.cropper.n_frames} frames")
        except Exception as e:
    ...
    def _analyze_frame(self, fov_idx: int, frame_idx: int, min_size: int = 15) -> list[dict[str, int]]:
        ...
        try:
            ...
            for pattern_idx in pattern_indices:
                try:
                    ...
                    records.append(
                        {"fov_index": fov_idx, "pattern_id": pattern_idx, "frame_index": frame_idx, "cell_count": -1}
                    )
                    continue
            ...
            for pattern_idx, n_count in zip(valid_pattern_indices, counts, strict=False):
                records.append(
                    {"fov_index": fov_idx, "pattern_id": pattern_idx, "frame_index": frame_idx, "cell_count": n_count}
                )
    ...
    def analyze(self, start_fov: int, end_fov: int, min_size: int = 15) -> dict[str, Any]:
        """
        ...
        if all_records:
            ...
            append_analysis_hdf5(self.h5_path, analysis_data)
