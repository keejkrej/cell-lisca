...
analyze_app = typer.Typer(help="Analyze cell counts for all patterns across all frames")
extract_app = typer.Typer(help="Extract cropped sequences from analyzed data")
...
def pattern_detect(
    ...
    from ..core.pattern import Patterner
...
def analyze_run(
    ...
    summary = analyzer.analyze(start_fov=start_fov, end_fov=end_fov, min_size=min_size)
...
def extract_run(
    ...
    summary = extractor.extract(n_cells=n_cells, tolerance_gap=tolerance_gap, min_frames=min_frames)

    typer.echo(f"Extraction complete: {summary['total_sequences']} sequences extracted")


# Graph commands
@graph_app.command("list-sequences")
def graph_list_sequences(
...
def graph_analyze(
    input: str = typer.Option(
        ..., "--input", "-i", help="Path to H5 file containing timelapse data with segmentation channel"
    ),
    output: str = typer.Option(..., "--output", "-o", help="Output directory for analysis results"),
    yaml_file: str | None = typer.Option(
        None, "--yaml", "-y", help="Path to corresponding YAML metadata file (optional, auto-detected if not provided)"
    ),
    fov_idx: int | None = typer.Option(None, "--fov", help="FOV index (required for H5 files)"),
    pattern_idx: int | None = typer.Option(None, "--pattern", help="Pattern index (required for H5 files)"),
    seq_idx: int | None = typer.Option(None, "--sequence", help="Sequence index (required for H5 files)"),
    start_frame: int | None = typer.Option(
        None, "--start-frame", "-s", help="Starting frame number (processes from first frame if not specified)"
    ),
    end_frame: int | None = typer.Option(
        None, "--end-frame", "-e", help="Ending frame number (exclusive, processes all frames if not specified)"
    ),
    search_radius: float = typer.Option(
        100.0, "--search-radius", help="Maximum search radius for cell tracking in pixels"
    ),
    tracking_config: str | None = typer.Option(
        None, "--tracking-config", help="Path to btrack config JSON file (optional, uses default if not specified)"
    ),
    adjacency_method: str = typer.Option(
        "boundary_length", "--adjacency-method", help="Method for building adjacency graphs"
    ),
    ...
    if is_h5:
        ...
        if validate_only:
            ...
            if loader.validate_cell_filter_output(input, yaml_file):
        ...
        loaded_data = loader.load_cell_filter_data(input, fov_idx, pattern_idx, seq_idx, yaml_file)
        ...
        loader = SegmentationLoader()
        loaded_data = loader.load_cell_filter_data(input, yaml_file)
        npy_path = input
        ...
        if is_h5:
            ...
            with tempfile.NamedTemporaryFile(suffix=".npy", delete=False) as tmp:
                ...
                combined_data = np.concatenate(
                    [loaded_data["data"], loaded_data["segmentation_masks"][:, np.newaxis, :, :]], axis=1
                )
...
# Tension command
@tension_app.command("run")
def tension_run(
    mask: str = typer.Option(..., "--mask", help="Path to segmentation mask .npy file"),
    output: str | None = typer.Option(None, "--output", help="Path to save the VMSI model (pickle)"),
